...
static int rcg_clk_set_duty_cycle(struct clk *c, u32 high, u32 period)
{
	struct rcg_clk *rcg = to_rcg_clk(c);
	unsigned long clkrate = clk_get_rate(c), newrate;
	...
	newrate = DIV_ROUND_CLOSEST(NSEC_PER_SEC, period);
	n_val = DIV_ROUND_CLOSEST(clkrate, newrate);
	...
	notn_m_val = (~(n_val - m_val)) & BM((rcg->mnd_reg_width - 1), 0);
	writel_relaxed(notn_m_val, N_REG(rcg));

	/* Calculate the 2d value */
	d_val = DIV_ROUND_CLOSEST((high * n_val * 2),  period);

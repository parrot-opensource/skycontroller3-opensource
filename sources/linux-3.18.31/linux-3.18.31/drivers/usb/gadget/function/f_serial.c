...
/* string descriptors: */
#define MAX_DESCRIPTOR_LENGTH	60
static char itf_descriptor[MAX_DESCRIPTOR_LENGTH] = "Generic Serial";
...
static ssize_t store_interface_descriptor(struct f_serial_opts *opts,
					const char *buf, size_t count)
{
	size_t len;

	strlcpy(itf_descriptor, buf, MAX_DESCRIPTOR_LENGTH);
	len = strnlen(itf_descriptor, MAX_DESCRIPTOR_LENGTH);

	/* Remove trailing newline */
	if (itf_descriptor[len - 1] == '\n')
		itf_descriptor[len - 1] = '\0';

	return strnlen(buf, count);
}

static ssize_t show_interface_descriptor(struct f_serial_opts *opts, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", itf_descriptor);
}

static struct f_serial_opts_attribute f_serial_descriptor =
	__CONFIGFS_ATTR(interface_descriptor, S_IRUGO | S_IWUSR,
			show_interface_descriptor, store_interface_descriptor);

static ssize_t store_interface_subclass(struct f_serial_opts *opts,
					const char *buf, size_t count)
{
	unsigned long val;
	...
	gser_interface_desc.bInterfaceSubClass = val;

	return count;
}

static ssize_t show_interface_subclass(struct f_serial_opts *opts, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%x\n",
			gser_interface_desc.bInterfaceSubClass);
}

static struct f_serial_opts_attribute f_serial_subclass =
	__CONFIGFS_ATTR(interface_subclass, S_IRUGO | S_IWUSR,
			show_interface_subclass, store_interface_subclass);

static struct configfs_attribute *acm_attrs[] = {
	&f_serial_port_num.attr,
	&f_serial_descriptor.attr,
	&f_serial_subclass.attr,
	NULL,
};

...
static struct notify_state {
	struct list_head	list;
	struct portmaster	*port;
	bool			connected;
} states[2*MAX_U_SERIAL_PORTS];

static void gser_notify_state(struct work_struct *data);
static DECLARE_WORK(gser_notify_work, gser_notify_state);

static LIST_HEAD(free_notify_queue);
...
#if defined(CONFIG_DEBUG_FS) && defined(CONFIG_USB_G_ANDROID)
...
void gserial_free_line(unsigned char port_num)
{
	...
	/* disable notify work*/
	cancel_work_sync(&gser_notify_work);
	...
	ports[port_num].tty_device = NULL;
...
int gserial_alloc_line(unsigned char *line_num)
{
	...
	ports[port_num].tty_device = tty_port_register_device(
			&ports[port_num].port->port,
			gs_tty_driver, port_num, NULL);

	if (IS_ERR(ports[port_num].tty_device)) {
		struct gs_port	*port;
		pr_err("%s: failed to register tty for port %d, err %ld\n",
				__func__, port_num,
				PTR_ERR(ports[port_num].tty_device));

		ret = PTR_ERR(ports[port_num].tty_device);
		port = ports[port_num].port;
		ports[port_num].port = NULL;
		ports[port_num].tty_device = NULL;
		gserial_free_port(port);
...
static void gser_notify_state(struct work_struct *data)
{
	struct notify_state *state = NULL, *snext = NULL;
	...
	if (list_empty(&notify_queue))
		return;

	spin_lock(&notify_lock);
	list_for_each_entry_safe(state, snext, &notify_queue, list) {
		list_move_tail(&state->list, &to_notify);
	}
	spin_unlock(&notify_lock);

	/* notify state change */
	list_for_each_entry_safe(state, snext, &to_notify, list) {
		if (!state)
			continue;

		tty_device = state->port->tty_device;
		envp_ext = state->connected ? connected : disconnected;

		if (WARN_ON(!tty_device))
			continue;

		kobject_uevent_env(&tty_device->kobj, KOBJ_CHANGE, envp_ext);
	}

	/* once notified, put state element in free_notify_queue */
	list_for_each_entry_safe(state, snext, &to_notify, list) {
		if (!state)
			continue;

		spin_lock(&notify_lock);
		list_move_tail(&state->list, &free_notify_queue);
		spin_unlock(&notify_lock);
	}
}
...
int gserial_connect(struct gserial *gser, u8 port_num)
{
	...
	int		status;
	struct device	*tty_device = NULL;
	struct notify_state *state = NULL;
	...
	tty_device = ports[port_num].tty_device;
	...
	if (tty_device) {
		...
		if (!list_empty(&free_notify_queue)) {
			state = list_first_entry(&free_notify_queue,
						struct notify_state, list);

			state->connected = 1;
			state->port  = &ports[port_num];

			list_move_tail(&state->list, &notify_queue);
			spin_unlock_irqrestore(&notify_lock, flags);

			schedule_work(&gser_notify_work);
		} else {
			spin_unlock_irqrestore(&notify_lock, flags);
			pr_warn("no more free items to notify tty state.\n");
		}
	} else
		pr_warn("no reference to serial device %d.\n", port_num);
...
static int userial_init(void)
{
	...
	for (i = 0; i < 2*MAX_U_SERIAL_PORTS; i++){
		states[i].connected = 0;
		states[i].port = NULL;
		list_add_tail(&states[i].list, &free_notify_queue);
	}

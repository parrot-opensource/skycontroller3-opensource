...
static void msm_usb_phy_reset(struct msm_otg *motg)
{
	...
	case SNPS_FEMTO_PHY:
		...
		/* Program tuning parameters for PHY */
		seq = motg->pdata->phy_init_seq_override;
		if (seq) {
			while (seq[0] >= 0) {
				writel_relaxed(seq[1],
						motg->phy_csr_regs + seq[0]);
...
struct msm_otg_platform_data *msm_otg_dt_to_pdata(struct platform_device *pdev)
{
	...
	property = of_get_property(node, "qcom,hsusb-otg-phy-init-seq-override", &len);
	if (property && len) {
		pdata->phy_init_seq_override = devm_kzalloc(&pdev->dev, len, GFP_KERNEL);
		if (!pdata->phy_init_seq_override)
			return NULL;
		of_property_read_u32_array(node, "qcom,hsusb-otg-phy-init-seq-override",
